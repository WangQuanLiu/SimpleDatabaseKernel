...
#define GRAM_CREATE_POSI 79
	...
	*/
	...
	输出：输出得到后的文法
	*/
	string GrammaticalAnalysisFile::string_map_to_gram(const string &str)
	{
	...
	const string GrammaticalAnalysisFile::keywordTable[]{
	...
	const string GrammaticalAnalysisFile::symbolStringTable[SYMBOL_SIZE]{
	...
	输出：离散之后产生的数字
	*/
	inline  int GrammaticalAnalysisFile::string_hash(const string &str)
	{
		int sum = 0;
		int i;
		for (i = 0; i < str.size(); i++)
